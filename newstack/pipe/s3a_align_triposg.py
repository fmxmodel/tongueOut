...
GATED (dies loudly, never ships a mis-aligned clay):
  - trimmed inlier RMSE  <= --max-rms cm
  - fitted-ICT 68-landmark-vertex -> aligned-SG-surface mean distance
    <= --max-face-dist cm  (the face MUST register, not just the blob)
...
from common import P, die, out_dir, save_json, umeyama  # noqa: E402
...
def main():
    ...
    ap.add_argument("--max-rms", type=float, default=1.2,
                    help="cm; GATE on trimmed inlier RMSE")
    ap.add_argument("--max-face-dist", type=float, default=1.2,
                    help="cm; GATE on mean fitted-landmark->SG-surface distance")
    ...
    d_all, _ = tree.query(src @ R_f.T * s_f + t_f, workers=-1)
    thr = np.quantile(d_all, 1.0 - args.trim)
    inlier = d_all <= thr
    ...
    # face gate: the fitted ICT landmark verts must lie ON the aligned surface
    ...
    lmk_mean, lmk_max = float(d_lmk.mean()), float(d_lmk.max())
    ...
    print(f"[s3sg] metrics: inlier_rms={inlier_rms:.3f} cm  median_nn="
          f"{median_nn:.3f} cm  median_rev={median_rev:.3f} cm")
    print(f"[s3sg] face landmarks -> SG surface: mean={lmk_mean:.3f} cm "
          f"max={lmk_max:.3f} cm")
    ...
    if inlier_rms > args.max_rms:
        die(f"ICP inlier RMSE {inlier_rms:.3f} cm > {args.max_rms} cm -- "
            "TripoSG alignment NOT trustworthy, refusing to ship it")
    if lmk_mean > args.max_face_dist:
        die(f"fitted-landmark->SG-surface mean {lmk_mean:.3f} cm > "
            f"{args.max_face_dist} cm -- the FACE did not register")
    if not (0.5 <= y_ratio <= 2.0):
        die(f"aligned SG y-extent ratio {y_ratio:.2f} vs ICT head is insane")
    ...
    info = {
        ...
        "trim": args.trim,
        "inlier_rms_cm": inlier_rms,
        "median_nn_cm": median_nn,
        "median_rev_cm": median_rev,
        "lmk_to_surface_mean_cm": lmk_mean,
        "lmk_to_surface_max_cm": lmk_max,
